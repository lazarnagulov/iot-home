...
def run_local_handler(
    ...
    dht_lcd_service: DhtLcdService, 
    rgb_ir_service: RgbIrService, 
    timer_service: KitchenTimerService, 
    stop_event: threading.Event
) -> None:
    threading.Thread(target = _handle_events, args=(event_bus, actuator_registry, dht_lcd_service, rgb_ir_service, timer_service, stop_event)).start()
    ...
    while not stop_event.is_set():
        ...
        if event is not None:
            try:
                ...
                elif event.sensor == "btn":
                    timer_service.handle_display_state(event.sensor, event.payload["name"])
