...
def run_gyroscope(config: GyroscopeConfig, event_bus: EventBus, threads: List[threading.Thread], 
                  stop_event: threading.Event, pause_event: threading.Event) -> None:
    if config.simulated:
        logger.info(f"Starting {config.id} Simulator")
        gyroscope_thread = threading.Thread(target = run_gyroscope_simulator, args=(config, 2, event_bus, stop_event, pause_event))
