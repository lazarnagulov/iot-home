...
def run_pir(config: PIRConfig, event_bus: EventBus, threads: List[threading.Thread], 
            stop_event: threading.Event, pause_event: threading.Event) -> None:
    if config.simulated:
        logger.info(f"Starting {config.id} Simulator")
        pir_thread = threading.Thread(target = run_pir_simulator, args=(config, 2, event_bus, stop_event, pause_event))
        pir_thread.start()
